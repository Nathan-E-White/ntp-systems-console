...
describe('prepared demo cases', () => {
    it('keeps the benchmark above its wall criterion while retaining a screening posture', () => {
        ...
        expect(outputs.thermalMarginK).toBeGreaterThan(0);
        expect(outputs.stabilityStatus).toBe(DEMO_CASES.baselineStartup.expectedPosture);
    });
    ...
    it('makes thermal margin the controlling concern in the investigation case', () => {
        ...
        expect(outputs.thermalMarginK).toBeLessThan(80);
        expect(outputs.stabilityStatus).toBe(DEMO_CASES.thermalMarginInvestigation.expectedPosture);
        expect(review.controllingConcern).toContain('exceeds');
    });

    it('parses all bundled model-evidence fixtures at startup', () => {
        expect(DEFAULT_ANALYSIS_EVIDENCE).toHaveLength(4);
    ...
    it('labels manual edits as a what-if without claiming fixture reruns', () => {
        ...
        expect(review.customerObjective).toContain('without implying');
